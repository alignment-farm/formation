...
from contact import balanced_relation_staged_validation as subject
...
def test_balanced_worlds_and_frozen_budget():
    ...
    assert counts == {FIRST_INCREASES: 3, SECOND_INCREASES: 3}
    assert subject.AUTHORSHIP_CALLS == 36
    assert subject.LATER_CALLS == 576
    assert subject.PLANNED_LOGICAL_CALLS == 612
    assert subject.PHYSICAL_CALL_CEILING == 620
...
def test_fake_packet_supports_and_replays(tmp_path):
    ...
    assert packet["logical_calls"] == packet["physical_attempts"] == 612
    assert packet["matching_scores"][subject.prior.EXPOSED] == 36
    assert packet["matching_scores"][subject.prior.WITHHELD] == 18
    assert packet["scope_errors_prevented"] == 18
...
def test_malformed_output_is_not_engaged(tmp_path):
    ...
    assert packet["logical_calls"] == 612


def test_correct_withheld_lessons_collapse_support(tmp_path):
    def leaked_withheld(body: bytes):
        user, record = request_record(body)
        if (
    ...
    packet = subject.execute(leaked_withheld, tmp_path / "collapsed")
    assert packet["matching_scores"][subject.prior.WITHHELD] == 36
